...
class StudentService {
  ...
  static Future<CRUDReturn> addStudentSubject(AddStudentSubjectDto<int> request) async {
    final response = await HttpService.patch('$_baseUrl/students/addSubject', body: request.toJson());
    return CRUDReturn.fromJson(jsonDecode(response.body));
  }

  static Future<CRUDReturn> addStudentSubjects(AddStudentSubjectDto<List<Subject>> request) async {
    final response = await HttpService.patch('$_baseUrl/students/addSubjects', body: request.toJson());
    return CRUDReturn.fromJson(jsonDecode(response.body));
  }

  static Future<CRUDReturn> removeStudentSubject(AddStudentSubjectDto<int> request) async {
    final response = await HttpService.patch('$_baseUrl/students/removeSubject', body: request.toJson());
    return CRUDReturn.fromJson(jsonDecode(response.body));
  }

  static Future<CRUDReturn> removeStudentSubjects(AddStudentSubjectDto<List<Subject>> request) async {
    final response = await HttpService.patch('$_baseUrl/students/removeSubjects', body: request.toJson());
    return CRUDReturn.fromJson(jsonDecode(response.body));
  }

  static Future<CRUDReturn> getStudentClasses(DateTime date, int studentNo) async {
    final response = await HttpService.get('$_baseUrl/students/classesByDate?date=$date&studentNo=$studentNo');
    return CRUDReturn.fromJson(jsonDecode(response.body));
  }

  static Future<CRUDReturn> getStudentClassesByDay(DateTime  date, int studentNo) async {
